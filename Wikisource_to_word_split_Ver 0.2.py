...
def extract_all_pages_to_json(book_title):
    ...
    if book_title.startswith('அட்டவணை:'):
        ...
        if category_page_count_match:
            ...
            total_pages = 200 # Assuming a maximum of 200 pages if not specified
        ...
        if total_pages > 0:
            for i in range(1, total_pages + 1):
                ...
                # else:
                    # print(f"எச்சரிக்கை: பக்கம் '{page_title}' காணப்படவில்லை. இது தவிர்க்கப்படுகிறது.")
            if pages_to_process:
                print(f"'{book_title}' நூலில் மொத்தம் {len(pages_to_process)} பக்கங்கள் கண்டறியப்பட்டன.")
            else:
