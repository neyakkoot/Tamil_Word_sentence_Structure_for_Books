...
import json
import re
import mwparserfromhell # Ensure mwparserfromhell is imported
...
# from pywikibot.proofreadpage import IndexPage # Not reliably providing page iteration, reverting to manual parsing
...
def extract_all_pages_to_json(book_title):
    ...
    main_page = Page(site, book_title) # Always get the main page first

    if not main_page.exists():
        print(f"பிழை: '{book_title}' என்ற பெயரில் நூல் ஏதும் காணப்படவில்லை.")
        return

    if book_title.startswith('அட்டவணை:'):
        print(f"'{book_title}' ஒரு அட்டவணை நூல். அதன் உள்ளடக்கத்தை ஆராய்ந்து உட்பக்கங்கள் பிரித்தெடுக்கப்படுகின்றன...")
        index_page_content = main_page.text # Get the raw text of the index page

        # Extract filename from the book_title itself (e.g., 'அட்டவணை:ஆடரங்கு.pdf' -> 'ஆடரங்கு.pdf')
        if ':' in book_title:
            file_name_for_pages = book_title.split(':', 1)[1].strip()
        else:
            file_name_for_pages = book_title.strip()

        total_pages = 0
        # Try to find total pages from categories like [[பகுப்பு:151 முதல் 200 வரை பக்கங்களுள்ள அட்டவணைகள்]]
        category_page_count_match = re.search(r'\[\[பகுப்பு:\d+ முதல் (\d+) வரை பக்கங்களுள்ள அட்டவணைகள்\]\]', index_page_content)
        if category_page_count_match:
            total_pages = int(category_page_count_match.group(1))
            print(f"கண்டுபிடிக்கப்பட்ட மொத்த பக்கங்கள் (வகை பிரிப்பில் இருந்து): {total_pages}")
        else:
            print("எச்சரிக்கை: பக்கங்களின் எண்ணிக்கை வகை பிரிவில் காணப்படவில்லை. ஒரு குறிப்பிட்ட எண்ணிக்கையிலான பக்கங்களைச் சோதிக்கிறேன்.")
            # Dynamic page discovery if category count not found or is insufficient
            discovered_pages_count = 0
            max_pages_to_check = 2000 # Safety limit

            print(f"'{book_title}' நூலுக்கான பக்கங்களை கண்டறியும் செயல்முறை தொடங்குகிறது...")
            for i in range(1, max_pages_to_check + 1):
                page_title = f"Page:{file_name_for_pages}/{i}"
                p = Page(site, page_title)
                if p.exists():
                    pages_to_process.append(p)
                    discovered_pages_count += 1
                else:
                    # If a page in sequence doesn't exist, assume we've reached the end
                    print(f"பக்கம் '{page_title}' காணப்படவில்லை. பக்கங்கள் கண்டறிதல் நிறுத்தப்பட்டது.")
                    break
            
            if discovered_pages_count > 0:
                print(f"'{book_title}' நூலில் மொத்தம் {discovered_pages_count} பக்கங்கள் கண்டறியப்பட்டன.")
                # Use discovered_pages_count as total_pages for reporting if dynamically found
                total_pages = discovered_pages_count 
            else:
                print(f"எச்சரிக்கை: '{book_title}' அட்டவணை நூலில் செல்லுபடியாகும் பக்கங்கள் கண்டறியப்படவில்லை.")

        # Original logic for iterating based on total_pages from category (now handled by dynamic discovery fallback)
        # This block is now mostly redundant if dynamic discovery is robust, but kept for clarity
        if total_pages > 0 and not pages_to_process: # Only if dynamic discovery didn't populate it and we have a count
    ...
    for p in pages_to_process:
        ...
        for sentence in sentences:
            ...
            for word in words:
                ...
                cleaned_word = clean_tamil_word(word_lower)
    ...
    safe_name = book_title.replace('/', '_').replace(' ', '_').replace(':', '_')
